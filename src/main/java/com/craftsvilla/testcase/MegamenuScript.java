...
public class MegamenuScript {
	...
	public static void megamenuScript() {
		...
		WebDriver driver = DriverSelector.getDriver_browserstacknew();
		for (int i = 1; i < listmultilab.size(); i++) {
			try {
				...
				if (responseCode == 200) {
					...
					driver.get(listmultilab.get(i).getLinks());
					Wait.defaultHighWait(driver);
					System.out.println(
							"count is" + DriverActions.getText(driver, ObjectRepository_HomePage.span_totalProduct));
					...
					System.out.println();
					requestSender.writeDataToTextFile(listmultilab.get(i).getSubCategory(),
							listmultilab.get(i).getLinks());
					mailsend = true;
